...
const EmergencyFAB = () => {
    ...
    const fetchDutyStaff = async () => {
        try {
            ...
            const res = await axios.get('https://vet-clinic-backend-tgtd.onrender.com/api/v1/hrm/duty-staff', {
    ...
    const fetchAllDoctors = async () => {
        try {
            const token = sessionStorage.getItem('token');
            const res = await axios.get('https://vet-clinic-backend-tgtd.onrender.com/api/v1/users?role=DOCTOR', {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (res.data.success) {
