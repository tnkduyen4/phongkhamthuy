...
const Sidebar = ({ isOpen, onClose }) => {
    ...
    React.useEffect(() => {
        ...
        const fetchGrooming = async () => {
            try {
                const token = sessionStorage.getItem('token');
                const res = await fetch('http://localhost:5000/api/v1/grooming', {
                    headers: { Authorization: `Bearer ${token}` }
                });
                const data = await res.json();
                if (data.success) {
    ...
    React.useEffect(() => {
        ...
        const fetchPending = async () => {
            try {
                ...
                const aptRes = await fetch('http://localhost:5000/api/v1/appointments', {
                    headers: { Authorization: `Bearer ${token}` }
                });
                ...
                const grRes = await fetch('http://localhost:5000/api/v1/grooming', {
                    headers: { Authorization: `Bearer ${token}` }
                });
