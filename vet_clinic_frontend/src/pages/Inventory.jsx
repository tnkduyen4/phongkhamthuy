...
const Inventory = () => {
    ...
    const fetchInventory = async () => {
        try {
            ...
            const res = await axios.get('http://localhost:5000/api/v1/inventory/medicines', {
    ...
    const handleViewTransactions = async (medicine) => {
        ...
        try {
            const token = sessionStorage.getItem('token');
            const res = await axios.get(`http://localhost:5000/api/v1/inventory/transactions?medicineId=${medicine._id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (res.data.success) {
    ...
    const handleConfirmImport = async () => {
        ...
        try {
            const token = sessionStorage.getItem('token');
            const res = await axios.post('http://localhost:5000/api/v1/inventory/medicines/bulk', { medicines: previewData }, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const token = sessionStorage.getItem('token');
            const url = editingId ? `http://localhost:5000/api/v1/inventory/medicines/${editingId}` : 'http://localhost:5000/api/v1/inventory/medicines';
    ...
    const handleProductSubmit = async (e) => {
        ...
        try {
            const token = sessionStorage.getItem('token');
            const url = editingProductId ? `http://localhost:5000/api/v1/products/${editingProductId}` : 'http://localhost:5000/api/v1/products';
    ...
    const confirmDelete = async (id) => {
        try {
            ...
            const checkRes = await axios.get(`http://localhost:5000/api/v1/inventory/medicines/${id}/check-delete`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            if (checkRes.data.hasRelations) {
                showConfirm(
                    ...
                    async () => {
                        try {
                            const res = await axios.delete(`http://localhost:5000/api/v1/inventory/medicines/${id}`, { headers: { Authorization: `Bearer ${token}` } });
                            if (res.data.success) {
                ...
                showConfirm(
                    ...
                    async () => {
                        try {
                            const res = await axios.delete(`http://localhost:5000/api/v1/inventory/medicines/${id}?force=true`, { headers: { Authorization: `Bearer ${token}` } });
                            if (res.data.success) {
                                setSelectedIds(prev => prev.filter(selectedId => selectedId !== id));
        ...
        showConfirm(
            ...
            async () => {
                try {
                    setLoading(true);
                    const token = sessionStorage.getItem('token');
                    const res = await axios.post(`http://localhost:5000/api/v1/inventory/medicines/bulk-delete`, { ids: selectedIds }, {
                        headers: { Authorization: `Bearer ${token}` }
                    });
                    if (res.data.success) {
                        setSelectedIds([]);
